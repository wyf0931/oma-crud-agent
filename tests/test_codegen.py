...
EXPECTED_FILES = [
    "app.py", "models.py", "views.py", "config.py", "extensions.py",
    "run.py", "run.sh", "pyproject.toml", ".gitignore", ".env.example",
    "README.md", "project.json", "instance",
]
...
def test_agent_codegen_node_uses_code_generator():
    """Verify agent/nodes/codegen.py delegates to CodeGenerator."""
    from agent.nodes.codegen import generate_project

    src = Path(__file__).parent.parent / "agent" / "nodes" / "codegen.py"
    content = src.read_text()
    assert "from codegen.generator import CodeGenerator" in content
    assert "generator.generate(" in content
    assert "CodeGenerator()" in content


def test_agent_codegen_node_with_minimal_state(tmp_path):
    """End-to-end: agent codegen node with minimal state."""
    from agent.nodes.codegen import generate_project

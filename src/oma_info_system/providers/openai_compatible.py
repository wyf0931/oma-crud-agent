...
from oma_info_system.platform.trace import trace_event
...
class OpenAICompatibleProvider:
    ...
    @staticmethod
    def _load_response_format() -> dict | None:
        ...
        try:
            ...
            raise ValueError("OPENAI_RESPONSE_FORMAT must be empty, json_object, or valid JSON.") from exc
        if not isinstance(parsed, dict) or not parsed.get("type"):
            raise ValueError("OPENAI_RESPONSE_FORMAT must be a JSON object with a type field.")
        return parsed

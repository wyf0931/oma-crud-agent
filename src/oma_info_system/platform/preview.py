...
class PreviewRecord:
    ...
    started_at: str | None = None  # ISO timestamp when process started
    stopped_at: str | None = None  # ISO timestamp when process stopped
    timeout_at: str | None = None  # ISO timestamp when will timeout
    last_activity: str | None = None  # ISO timestamp of last user interaction
...
class PreviewManager:
    ...
    def _save(self) -> None:
        ...
        data = {record_id: record.to_dict() for record_id, record in cleaned_records.items()}
        try:
            with open(self.storage_path, "w") as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
        except OSError as exc:
            logger.warning("Could not persist preview state to %s: %s", self.storage_path, exc)
    ...
    def _cleanup_records(self) -> dict[str, PreviewRecord]:
        ...
        return {
            ...
            or (r.state == "destroyed" and datetime.fromisoformat(r.stopped_at or r.created_at) >= cutoff)
        }

    def start_preview(self, project_id: str, project_path: str, port: int, process: subprocess.Popen) -> PreviewRecord:
    ...
    def get_record(self, project_id: str) -> PreviewRecord | None:
    ...
    def start_cleanup_thread(
            ...
            while not self._stop_cleanup.is_set():
                try:
                    ...
                    if timeout_cleaned > 0 or zombie_cleaned > 0:
                        print(
                            f"[PreviewManager] Cleaned {timeout_cleaned} timed out, {zombie_cleaned} zombie processes"
                        )
        ...
        self._cleanup_thread = threading.Thread(target=cleanup_loop, daemon=True, name="preview_cleanup")
...
def get_preview_manager() -> PreviewManager:
    ...
    if _preview_manager is None:
        ...
        _preview_manager = PreviewManager(storage_path=str(storage_path), timeout_minutes=30)
